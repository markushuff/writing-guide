...
- See Chapter 7 for how to handle requests to add theoretical coverage
:::
...
When submitting a revision, most journals ask for a version showing what was altered. Before opening the manuscript, save a copy under a new filename — `paper_revised.docx` or similar. Never revise the original file. If a co-author or editor later asks what changed, you need an untouched baseline to compare against.

With the copy open, enable track changes before writing a single word. A few further rules:
...
**Desk rejection** usually signals one of three things: the paper is outside the journal's scope, the contribution is not seen as significant enough for that venue, or the writing did not communicate the contribution clearly enough. The first is a targeting problem. The second and third are fixable.
